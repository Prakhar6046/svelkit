...
<aside
	class={`fixed top-0 left-0 z-20 h-full overflow-hidden border-r bg-bland-25 duration-300
          ${$showDashboardSidebar ? 'w-64 border-r-bland-300/40 px-3  lg:w-64' : 'w-0 border-transparent'}
          mobile-sidebar-fix`}
>
	<img src={AppLogo} alt="Ship in a Flash Logo" class="mt-3 cursor-pointer" />

	<ul class="mt-8 flex flex-col gap-1.5">
		<!-- Dashboard -->
		<li
			onclick={() => {
				navigate('/dashboard');
		...
		<!-- Pickups (Collapsible) -->
		<!-- Pickups (Collapsible) -->
		<li class="relative rounded-md text-sm font-medium">
			<Collapsible bind:open={pickupsOpen}>
				<CollapsibleTrigger
					class={`flex w-full cursor-pointer items-center gap-4 rounded-md p-2 text-left text-sm font-medium duration-100 
        ${
				...
				>
					...
					<Icon
						icon="uil:angle-down"
						class="size-6 transition-transform duration-300 {pickupsOpen ? 'rotate-180' : ''}"
					/>
				</CollapsibleTrigger>
